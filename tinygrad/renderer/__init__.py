    ...
    if ignore_indexing:
      ...
      for u in uops:
        if u.op in {Ops.LOAD, Ops.STORE}:
          ...
          dont_count.update((UOp.sink(*u.src[0].src[1:]) if u.src[0].op is Ops.INDEX else u.src[0]).toposort(range_gate))
          # TODO: is this correct? this all needs to be cleaned up
          if len(u.src) > 2: dont_count.update(u.src[2].toposort())
        elif u.op is Ops.IF:
          dont_count.update(u.src[0].toposort())
    for u in uops:
      if u.op in {Ops.LOAD, Ops.STORE}:
        buf = u
