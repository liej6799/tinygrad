...
void preload_init() {
  Py_Initialize();
  PyRun_SimpleString("print('hello from c'); import tinygrad.extra.hook");
}
...
int ioctl(int fd, unsigned long request, void *arg) {
	...
	// Import the module
	pName = PyUnicode_FromString("tinygrad.extra.hook");
	...
	if (pModule != NULL) {
		...
		Py_DECREF(pModule);
			...
			PyErr_Print();
		fprintf(stderr, "Failed to load 'tinygrad.extra.hook'\n");
	}
